...
module: build
	...
	tar czf $(BIN_OUTPUT_PATH)/module.tar.gz $(BIN_OUTPUT_PATH)/hardkernel meta.json

build: build-go

build-go:
	rm -f $(BIN_OUTPUT_PATH)/hardkernel
	go build -tags no_cgo,osusergo,netgo -ldflags="-extldflags=-static $(COMMON_LDFLAGS)"  -o $(BIN_OUTPUT_PATH)/hardkernel main.go
